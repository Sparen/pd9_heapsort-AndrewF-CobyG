...
/*****
 * HeapSort
 * Best runtime: O(nlogn) - Array already in order, so add runs O(logn), removeMin O(nlogn)
 * Worst runtime: O(nlogn) - Array in reverse order, so add runs O(nlogn), removeMin O(nlogn)
 * (runtimes are most likely completely off)
 *****/ 
...
public class HeapSort {
	...
	Integer[] returnArr = new Integer[data.length];
	ALHeap heapity = new ALHeap();
    ...
    public static void printArray(Integer[]a){ //--Sparen, not needed.
	String result = "[ ";
	...
	Integer[] lulz = {5,2,8,3,6,2,7,45,2,3,6,4,6,34,7,3,63542,24,2,5,23,3};
	printArray(lulz);
	lulz = sort(lulz);
	printArray(lulz);
